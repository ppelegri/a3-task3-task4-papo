...
ggplot(data = urchins_clean, aes(x = size, fill = common_name)) + # histogram will be based on urchin size, colors according to species
...
urchins_stats <- urchins_clean %>% # start with urchins_clean
   ...
   summarize(mean = round(mean(size, na.rm = TRUE), 1), # find mean size and round to 1 decimal point
             sd = round(sd(size, na.rm = TRUE), 1), # find sd of size and round to 1 decimal point
             median = median(size, na.rm = TRUE), # find median size
             sample_size = n() # count sample size
   )

kable(urchins_stats, # create table
      col = c("Species", "Mean Size (cm)", "Standard deviation", "Median Size (cm)", "Sample Size"), # list column names
      caption = "Table 1: Summary statistics on size of puruple and red urchins") %>% 
